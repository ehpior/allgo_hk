...
for i, portfolio in enumerate(d):
    ...
    if dates[p_holding_day - 1] == portfolio['first_buy_date']:
        price = abs(float(db_redis.get('stock:' + portfolio['code'])))

        final_data.append((portfolio['id'], portfolio['id'], portfolio['code'], portfolio['stock_name'],
                           today, price, portfolio['holding_day']))

        print(f'[id : {portfolio["id"]}, code : {portfolio["code"]}, stock_name : {portfolio["stock_name"]}, '
              f'price : {price}, holding_day : {portfolio["holding_day"]}]')
